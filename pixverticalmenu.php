...
require(dirname(__FILE__).'/pixverticalmenutoplinks.class.php');

class PixVerticalMenu extends Module implements WidgetInterface
{
    ...
    {
        $this->name = 'pixverticalmenu';
        $this->tab = 'front_office_features';
        $this->version = '1.0.4';
        $this->author = 'PixelStrap';

        $this->bootstrap = true;
        parent::__construct();

        $this->displayName = $this->l('PIX - Vertical Menu');
    ...
    public function install($delete_params = true)
    {
        ...
        if ($delete_params) {
            if (!$this->installDb() || !Configuration::updateGlobalValue('MOD_PIXVERTICALMENU_ITEMS', 'CAT3')) {
                return false;
            }
        }
    ...
    public function installDb()
    {
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pixverticalmenu` (
			`id_pixverticalmenu` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`new_window` TINYINT( 1 ) NOT NULL,
			INDEX (`id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;') &&
            Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pixverticalmenu_lang` (
			`id_pixverticalmenu` INT(11) UNSIGNED NOT NULL,
			`id_lang` INT(11) UNSIGNED NOT NULL,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`label` VARCHAR( 128 ) NOT NULL ,
			`link` VARCHAR( 128 ) NOT NULL ,
			INDEX ( `id_pixverticalmenu` , `id_lang`, `id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;'));
    }
    ...
    public function uninstall($delete_params = true)
    {
        ...
        if ($delete_params) {
            if (!$this->uninstallDB() || !Configuration::deleteByName('MOD_PIXVERTICALMENU_ITEMS') || !Configuration::deleteByName('MOD_PIXVERTICALMENU_COLS')) {
                return false;
            }
        }
    ...
    {
        Db::getInstance()->execute('DROP TABLE IF EXISTS`'._DB_PREFIX_.'pixverticalmenu`');
        Db::getInstance()->execute('DROP TABLE IF EXISTS`'._DB_PREFIX_.'pixverticalmenu_lang`');
        return true;
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitpixverticalmenu')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                if (count($shops) == 1) {
                    if (is_array($items) && count($items)) {
                        $updated = Configuration::updateValue('MOD_PIXVERTICALMENU_ITEMS', (string)implode(',', $items), false, (int)$shop_group_id, (int)$shop_id);
                    } else {
                        $updated = Configuration::updateValue('MOD_PIXVERTICALMENU_ITEMS', '', false, (int)$shop_group_id, (int)$shop_id);
                    }
                }
				
				$number_cols = Tools::getValue('number_cols_menu');
				$updated &= Configuration::updateValue('MOD_PIXVERTICALMENU_COLS', (string)$number_cols, false, (int)$shop_group_id, (int)$shop_id);
            ...
            if (Tools::isSubmit('submitpixverticalmenuLinks')) {
                ...
                if ($count_links_label || $count_label) {
                    if (!$count_links_label) {
                        ...
                        foreach ($shops as $shop_id) {
                            $added = PixVerticalMenuTopLinks::add($links_label, $labels,  Tools::getValue('new_window', 0), (int)$shop_id);
                ...
                $update_cache = true;
            } elseif (Tools::isSubmit('deletepixverticalmenu')) {
                $errors_delete_link = array();
                $id_pixverticalmenu = Tools::getValue('id_pixverticalmenu', 0);
                $shops = Shop::getContextListShopID();

                foreach ($shops as $shop_id) {
                    $deleted = PixVerticalMenuTopLinks::remove($id_pixverticalmenu, (int)$shop_id);
                    Configuration::updateValue('MOD_PIXVERTICALMENU_ITEMS', str_replace(array('LNK'.$id_pixverticalmenu.',', 'LNK'.$id_pixverticalmenu), '', Configuration::get('MOD_PIXVERTICALMENU_ITEMS')));
                ...
                $update_cache = true;
            } elseif (Tools::isSubmit('updatepixverticalmenu')) {
                $id_pixverticalmenu = (int)Tools::getValue('id_pixverticalmenu', 0);
                ...
                if (Tools::isSubmit('updatelink')) {
                    ...
                    Tm_VerticalMenuTopLinks::update($link, $label, $new_window, (int)$id_shop, (int)$id_pixverticalmenu, (int)$id_pixverticalmenu);
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, PixVerticalMenuTopLinks::gets((int)$id_lang, null, (int)$shop_id));
        }
        ...
        if (is_array($items) && count($items)) {
            ...
            if (count($shops) > 1) {
                foreach ($shops as $key => $shop_id) {
                    $shop_group_id = Shop::getGroupFromShop($shop_id);
                    $conf .= (string)($key > 1 ? ',' : '').Configuration::get('MOD_PIXVERTICALMENU_ITEMS', null, $shop_group_id, $shop_id);
                }
            } else {
                $shop_id = (int)$shops[0];
                $shop_group_id = Shop::getGroupFromShop($shop_id);
                $conf = Configuration::get('MOD_PIXVERTICALMENU_ITEMS', null, $shop_group_id, $shop_id);
            }
    ...
    protected function makeMenuOption()
    {
        ...
        foreach ($menu_item as $item) {
            ...
            switch (Tools::substr($item, 0, Tools::strlen($values[1]))) {
                ...
                case 'LNK':
                    $link = Tm_VerticalMenuTopLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = PixVerticalMenuTopLinks::get($link[0]['id_pixverticalmenu'], (int)$default_language, (int)Shop::getContextShopID());
                        }
                        $html .= '<option selected="selected" value="LNK'.(int)$link[0]['id_pixverticalmenu'].'">'.Tools::safeOutput($link[0]['label']).'</option>';
                    }
                    break;
		...
		foreach ($menu_items as $item) {
            ...
            switch (Tools::substr($item, 0, Tools::strlen($value[1]))) {
                ...
                case 'SHOP':
                    ...
                    break;
                case 'LNK':
                    $link = PixVerticalMenuTopLinks::get($id, $id_lang, $id_shop);
                    if (!empty($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = pixVerticalMenuTopLinks::get($link[0]['id_pixverticalmenu'], $default_language, (int)Shop::getContextShopID());
                        }
                        $root_node['children'][] = $this->makeNode([
                            'type' => 'link',
    ...
    protected function generateCategoriesMenu($categories, $is_children = 0)
    {
        ...
        foreach ($categories as $key => $category) {
            ...
            $node['url'] = $link;
            $node['type'] = 'category';
            $node['page_identifier'] = 'pixcategory-' . $category['id_category'];
    ...
    {
        return _PS_CACHE_DIR_ . DIRECTORY_SEPARATOR . 'pixverticalmenu';
    }
    ...
    {
        $pixverticalmenu = Db::getInstance()->executeS('
			SELECT *
			FROM '._DB_PREFIX_.'pixverticalmenu
			WHERE id_shop = '.(int)$params['old_id_shop']
            );

        foreach ($pixverticalmenu as $id => $link) {
            Db::getInstance()->execute('
				INSERT IGNORE INTO '._DB_PREFIX_.'pixverticalmenu (id_pixverticalmenu, id_shop, new_window)
				VALUES (null, '.(int)$params['new_id_shop'].', '.(int)$link['new_window'].')');

            $pixverticalmenu[$id]['new_id_pixverticalmenu'] = Db::getInstance()->Insert_ID();
        }

        foreach ($pixverticalmenu as $id => $link) {
            $lang = Db::getInstance()->executeS('
					SELECT id_lang, '.(int)$params['new_id_shop'].', label, link
					FROM '._DB_PREFIX_.'pixverticalmenu_lang
					WHERE id_pixverticalmenu = '.(int)$link['id_pixverticalmenu'].' AND id_shop = '.(int)$params['old_id_shop']);

            foreach ($lang as $l) {
                Db::getInstance()->execute('
					INSERT IGNORE INTO '._DB_PREFIX_.'pixverticalmenu_lang (id_pixverticalmenu, id_lang, id_shop, label, link)
					VALUES ('.(int)$link['new_id_pixverticalmenu'].', '.(int)$l['id_lang'].', '.(int)$params['new_id_shop'].', '.(int)$l['label'].', '.(int)$l['link'].' )');
            }
        }
    }
    ...
    public function renderForm()
    {
        ...
        $helper->tpl_vars = array(
			...
			//'MOD_pixverticalmenu_COLS' => $this->getNumberColsMenu(),
        );
    ...
    public function renderAddForm()
    {
        $fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => (Tools::getIsset('updatepixverticalmenu') && !Tools::getValue('updatepixverticalmenu')) ?
                ...
                'submit' => array(
                    'name' => 'submitpixverticalmenuLinks',
                    'title' => $this->l('Add')
                )
            ),
        );
        ...
        if (Tools::getIsset('updatepixverticalmenu') && !Tools::getValue('updatepixverticalmenu')) {
            $fields_form['form']['submit'] = array(
                'name' => 'updatepixverticalmenu',
                'title' => $this->l('Update')
            );
        }

        if (Tools::isSubmit('updatepixverticalmenu')) {
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'updatelink');
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_pixverticalmenu');
    ...
    public function renderChoicesSelect()
    {
        ...
        $links = PixVerticalMenuTopLinks::gets($this->context->language->id, null, (int)Shop::getContextShopID());
        foreach ($links as $link) {
            if ($link['label'] == '') {
                $default_language = Configuration::get('PS_LANG_DEFAULT');
                $link = PixVerticalMenuTopLinks::get($link['id_pixverticalmenu'], $default_language, (int)Shop::getContextShopID());
                if (!in_array('LNK'.(int)$link[0]['id_pixverticalmenu'], $items)) {
                    $html .= '<option value="LNK'.(int)$link[0]['id_pixverticalmenu'].'">'.$spacer.Tools::safeOutput($link[0]['label']).'</option>';
                }
            } elseif (!in_array('LNK'.(int)$link['id_pixverticalmenu'], $items)) {
                $html .= '<option value="LNK'.(int)$link['id_pixverticalmenu'].'">'.$spacer.Tools::safeOutput($link['label']).'</option>';
            }
        }
        $html .= '</optgroup>';
        $html .= '</select>';
    ...
    public function getAddLinkFieldsValues()
    {
        ...
        if (Tools::isSubmit('updatepixverticalmenu')) {
            $link = PixVerticalMenuTopLinks::getLinkLang(Tools::getValue('id_pixverticalmenu'), (int)Shop::getContextShopID());
        ...
        $fields_values = array(
            ...
            'id_pixverticalmenu' => Tools::getValue('id_pixverticalmenu'),
        );
    ...
    public function renderList()
    {
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, PixVerticalMenuTopLinks::gets((int)$this->context->language->id, null, (int)$shop_id));
        }

        $fields_list = array(
            'id_pixverticalmenu' => array(
        ...
        $helper->identifier = 'id_pixverticalmenu';
        $helper->table = 'pixverticalmenu';
    ...
    {
        $this->context->controller->registerStylesheet('modules-pixverticalmenu', 'modules/'.$this->name.'/views/css/pixverticalmenu.css', ['media' => 'all', 'priority' => 150]);
    }
    ...
    public function renderWidget($hookName, array $configuration)
    {
        ...
        return $this->fetch('module:pixverticalmenu/views/templates/hook/pixverticalmenu.tpl');
    }
	
	private function getNumberColsMenuItems()
	{
		$items_cols = Tools::getValue('MOD_PIXVERTICALMENU_COLS');
		...
		if (is_array($items_cols) && count($items_cols)) {
			...
			if (count($shops) > 1) {
				foreach ($shops as $key => $shop_id) {
					$shop_group_id = Shop::getGroupFromShop($shop_id);
					$conf .= ($key > 1 ? ',' : '').((string)Configuration::get('MOD_PIXVERTICALMENU_COLS', null, $shop_group_id, $shop_id));
				}
			} else {
				$shop_id = (int)$shops[0];
				$shop_group_id = Shop::getGroupFromShop($shop_id);
				$conf = Configuration::get('MOD_PIXVERTICALMENU_COLS', null, $shop_group_id, $shop_id);
			}
	...
	private function getNumberColsMenu()
	{
		...
		if (!empty($icon_values)) {
			...
			for ($i = 0; $i < $count_icon_values; $i++) {
				$html .= '<select class="number_cols_pixmegamenu" name="number_cols_'.$i.'" id="number_cols_'.$i.'">';
				for ($j = 1; $j <= 12; $j++) {
					$html .= '<option value="'.$j.'" ';
					if ($icon_values[$i] == $j)
						$html .= 'selected';
